...
the form $C(v\,; F_{x,t}, k, x): v\mapsto c$, where $c$ is the option price,
and $F_{x,t}, v, k, x$ are the risk neutral forward price at time $x$ of an
underlying contract of tenor $t$, its implied volatility,
the strike price of the option and
the option's time to expiry, respectively.
For at-the-money options, $k=F_{x,t}$.
Let $\mathbb{C}_v$ be the space of functions $C_v:(F_{x,t},k,x)\mapsto c$,
...
Put differently, $V(x,t,k)=C^{-1}(c;F_{x,t},k,x)$.
...
\section{Forward Stochastic model}\label{sec:sm}
The current common options market practice is to model the stochastic behavior of interest rate contracts
using the SABR model (for a full discussion of the SABR model and its simulation see Tawfik \cite{T1}).
In short, the SABR model is given by
...
The values of $\mu_{x,t}, \theta_{x,t}$ and $\nu_{x,t}$ are all non-negative.
...
\beqn
\hat{\nu}^2 = \frac{n}{t_n}\,\frac{\sum_{i<n} (\alpha_i-\alpha_{i-1})^2}{\sum_{i<n}\alpha_i}\,.
\eeqn

Ben Alaya and Kebaier \cite{BK} showed that

\beqn
\frac{1}{t_n}\,\int_0^{t_n} \alpha_s \,ds\,\xrightarrow{\;\;\mathbb{P}\;\;}\,\mu\quad \text{as} \quad t\!\longrightarrow\!\infty\,.
\eeqn

They also suggested the following for the consistent estimation of $\theta$

\beqn
\hat{\theta}\,=\,\frac{t_n\,(\log{\alpha_n}-\log{\alpha_0}+\frac{t_n}{2n}\,\nu^2\sum_{i<n}\frac{1}{\alpha_i})-\frac{t_n}{n}(\alpha_n-\alpha_0)\sum_{i<n}\frac{1}{\alpha_i}}
{t_n^2\,(\,\frac{1}{n^2}\sum_{i<n}\!\frac{1}{\alpha_i}\;\sum_{i<n}\!\alpha_i\;-\;1\,)}\,.
\eeqn

\section{Option Portfolio}
An at-the-money implied volatility surface is usually given with respect to a very large grid
made of a list of option expiries along one dimension, and a list of underlying tenors
along the other dimension.
Reduction of the underlying dimensionality is desired for two main reasons.
First, any realistic simulation of an option portfolio has to reflect the natural stochastic 
dependence inherent when modeling derivatives on highly correlated underlyings.
Secondly, full simulation of the entire grid, on a daily basis for long time horizons,
results in unjustifiable resource and time consumption.

In the next two subsections,
we propose two approaches for complexity reduction.
The first approach is based on relating the volatility of all options to the underlying
stochastic factors of the forward yield curve itself.
The second approach uses Principal Component Analysis (PCA) to
...
\beqn
S(t_x,t_s,t_e)=\frac{P(t_x,t_s)-P(t_x,t_e)}{A(t_x)}\,.
\eeqn
...
\beqn
\begin{aligned}
\sigma_{s,e,i}(t)=&\,\frac{P(t,t_s)-P(t,t_e)}{A^2}\sum_{j=0}^n\pi_i P(t,T_j)\,\bar{\sigma}_i(t,T_j)\,
-\,\frac{P(t,t_s)}{A(t)}\,\bar{\sigma}_n(t,t_s)+\,\frac{P(t,t_e)}{A(t)}\,\bar{\sigma}_n(t,t_e)\\[12.5pt]
=&\,\frac{1}{A(t)}\left(\frac{P(t,t_s)-P(t,t_e)}{A(t)}\sum_{j=0}^n\pi_i P(t,T_j)\,\bar{\sigma}_i(t,T_j)\,
-\,P(t,t_s)\,\bar{\sigma}_n(t,t_s)+\,P(t,t_e)\,\bar{\sigma}_n(t,t_e)\right)\\[12.5pt]
=&\,\frac{1}{A(t)}\left(S(t,t_s,t_e)\,\sum_{j=0}^n\pi_i P(t,T_j)\,\bar{\sigma}_i(t,T_j)\,
-\,P(t,t_s)\,\bar{\sigma}_n(t,t_s)+\,P(t,t_e)\,\bar{\sigma}_n(t,t_e)\right)
\end{aligned}
\eeqn
...
&-\left.\,P(t,t_s)\int_t^{t_s}\sigma_i(t,u)\,du+\,P(t,t_e)\int_t^{t_e}\sigma_i(t,u)\,du\right)\\
\sigma_{s,e,i}(t)=&\,\frac{S(t,t_s,t_e)}{A(t)}\,\sum_{j=0}^n\pi_i\, P(t,T_j)\int_t^{T_j}\sigma_i(t,u)\, du\\
&-\,S(t,t_s,t_e)\int_t^{t_s}\sigma_i(t,u)\,du+\,\frac{P(t,t_e)}{A(t)}\int_{t_s}^{t_e}\sigma_i(t,u)\,du\,.
\end{aligned}
\eeqn
...
With the aid of the semi-analytic formula, $H(\alpha|.)$ of Section \ref{sec:sm},
each simulated $\alpha_{x,t}$ can be converted to a simulated $V(x,t,k)$,
and subsequently, a simulated at-the-money implied volatility surface.
...
Discretization recursion \eqref{eq:disc} is guaranteed to converge, as long as,
$\nu^2<4\theta\mu$ and $\delta<2/\theta$.

\section{Arbitrage Testing and Fixing}
Johnson and Nonas \cite{JN} argued that the only arbitrage constraint that needs to be respected
by swaptions on the same rate is the triangular inequality, namely,
\beqn\label{eq:arb}
C(v_1;F_{x,t1},k,x)+C(v_2;F_{t1+x,t2},k,t1+x)\ge C(v_3;F_{x,t1+t2},k,x)\,.
\eeqn
It is possible that the simulation generated implied volatility surface
violates the condition \eqref{eq:arb}.
To test for any violation of condition \eqref{eq:arb} and to alleviate any such violation,
we offer the following algorithm.
\bigskip
\begin{lstlisting}
    assume a grid of $\alpha(x,t)$ values
    assume a grid of $V(x,t)$ values
    assume a function $H(k)$ that maps $\alpha(x,t)$ to $V(x,t,k)$
    let $h_x[x]$ be a hash table that returns the values $t$ for all valid $\alpha(x,t)$
    let $h_t[t]$ be a hash table that returns the values $x$ for all valid $\alpha(x,t)$
    for $x$ in $X$ (in increasing order):
100     for $t$ in $h_x[x]$ (in increasing order):
            $C1 = C(V(x,t,F_{x,t}))$
            for $t1$ in $h_x[t+x]$ (in increasing order):
                $C2 = C(V(x+t,t1,F_{x,t}))$
                if $\alpha(x,t+t1)$ exists:
                    $C3 = C(V(x,t+t1,F_{x,t}))$
                    if $C3 > C1 + C2$:
                        # arbitrage violation
                        # repair
                        set $\alpha(x,t)$ such that $V(x,t,F_{x,t})$=$V(x,t+t1,F_{x,t})$
                        set $\alpha(x+t,t1)$ such that $V(x+t,t1,F_{x,t})$=$V(x,t+t1,F_{x,t})$
                        goto 100
\end{lstlisting}

\section{Backtesting}
A deep discussion of several detailed approaches to backtesting are given by Tawfik in \cite{T}.
...
\begin{thebibliography}{99}
    ...
    \bibitem{BK}
        Ben Alaya, M. and Kebaier A, 2013, “Asymptotic Behavior of the Maximum Likelihood Estimator for Ergodic and Nonergodic Square-root Diffusions.” Stochastic Analysis and Applications, 31(4), 552-573. doi:10.1080/07362994.2013.798175.
    \bibitem{CW}
    ...
    \bibitem{JN}
        Johnson, Simon and Nonas, Bereshad, 2009, "Arbitrage-Free Construction Of The Swaption Cube,"
        https://ssrn.com/abstract=1330869
    \bibitem{P}
    ...
    \bibitem{T}
        Tawfik, Maged, 2025, "Monte Carlo Simulation of SOFR Yield Curve," Working Paper.
    \bibitem{T1}
        Tawfik, Maged, 2026, "SABR Simulation for Risk Management," Working Paper.
\end{thebibliography}
\end{document}