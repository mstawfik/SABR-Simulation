...
Throughout the document, the term SABR volatility will refer to $\sigma_t$ of
Equation~\eqref{eq:sabr}
...
\section{Implied Volatility}\label{sec:ivol}
...
\section{Asymptotic Representations}\label{sec:asym}
...
future time horizons, on a single option.
Such framework is useful for pricing options written at time $t_0$ 
with expiry that
matches any of the simulation horizon.
This assumes that the starting values of $\nu, \rho, \beta\,$
are known at time $t_0$ and either, remain constant, or are
given as known function in time.

A portfolio of options written on different contracts of the same
interest rate curve have to reflect the stochastic dependence
of the forward prices of such contracts.
Otherwise, simulation results may be totally unrealistic or even non arbitrage-free.
In this section we will address this issue as it pertains to a portfolio of
interest rate swaptions.

\subsection{SABR in a Portfolio Setting}
In a portfolio setting, we may choose to re-write Equation~\eqref{eq:sabr} as

\beqn
dF=\,&F_{t}^{\beta} \sigma_t dW_t\quad&\beta\in[0,1],\,F_0>0\\
\eeqn
...
or to history and use realized volatility as a proxy.
We propose the use of the options market implied volatilities to estimate the current
instantaneous forward volatilities.

The set of implied volatilities associated with the prices of a set of contemporaneous swaptions on the
same interest rate curve define what is called a {\em volatility cube}.
The underlying dimensions of the cube are: tenor, strike and expiry.
In Section~\ref{sec:ivol}, we discussed the various types of implied volatilities
and how they can be calculated from swaption prices.
In Section~\ref{sec:asym}, we showed how 
...
\section{CIR}
Milstein representation

The **Cox-Ingersoll-Ross (CIR)** process is the stochastic differential equation (SDE):

$$
dX_t = \kappa (\theta - X_t) \, dt + \sigma \sqrt{X_t} \, dW_t, \quad X_0 = x_0 \geq 0
$$

where $\kappa > 0$, $\theta > 0$, $\sigma > 0$ are constants, and $W_t$ is a standard Wiener process.

The **Milstein scheme** is a higher-order Itô–Taylor discretization (strong order 1.0 for Lipschitz SDEs,
but the CIR diffusion is only Hölder-1/2 continuous near zero, so special care is often needed).

- The term $\frac{\sigma^2}{4} (\Delta W^2 - \Delta t)$ comes from the Itô correction (second-order term).
- This scheme can become **negative** when $X_i$ gets close to zero (especially when the Feller condition $2\kappa\theta > \sigma^2$ is **not** satisfied).
- Many practical implementations therefore use **reflecting**, **truncating**, **absorption**, or **implicit** variants (e.g. θ-Milstein with θ ≥ 0.5, fully implicit Milstein, drift-implicit square-root Milstein, etc.) to preserve non-negativity and/or moment stability.
- For very accurate pathwise simulation when negativity must be avoided, people often prefer **exact simulation** (via non-central chi-squared transitions) or Lamperti-transformed schemes instead of plain Milstein.

So the **classical explicit Milstein representation** of the CIR process is:

$$
\boxed{
X_{i+1} = X_i + \kappa(\theta - X_i)\Delta t + \sigma\sqrt{X_i}\,\Delta W_i + \dfrac{\sigma^2}{4}\bigl((\Delta W_i)^2 - \Delta t\bigr)
}
$$

Let me know if you'd like the implicit versions, a truncated/reflecting variant, MATLAB/Python code snippet, or comparison with Euler / exact scheme!
\bigskip
